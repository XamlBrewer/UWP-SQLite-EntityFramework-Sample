    ...
    internal static class Repository
    {
        internal async static Task CreateDatabase()
        {
            using (var db = new PersonContext())
            {
                ...
                StorageFile file = await StorageFile.GetFileFromPathAsync(Path.Combine(Package.Current.InstalledLocation.Path, @"Assets\Pictures\GoT_2.png"));
                ...
                file = await StorageFile.GetFileFromPathAsync(Path.Combine(Package.Current.InstalledLocation.Path, @"Assets\Pictures\GoT_1.png"));
                person.Picture = await file.AsByteArray();
                i = db.People.Add(person);
